...
void FfiDispatcher::populate_memory_from_json(char* dest_ptr, const json& value_json, const std::string& type_name, FfiArgs& arg_storage) {
...
json FfiDispatcher::read_json_from_memory(const char* src_ptr, const std::string& type_name) const {
    ...
    else if (type_name == "uint8") return *reinterpret_cast<const uint8_t*>(src_ptr);
    else if (type_name == "int16") return *reinterpret_cast<const int16_t*>(src_ptr);
    else if (type_name == "uint16") return *reinterpret_cast<const uint16_t*>(src_ptr);
    else if (type_name == "int32") return *reinterpret_cast<const int32_t*>(src_ptr);
    else if (type_name == "uint32") return *reinterpret_cast<const uint32_t*>(src_ptr);
    else if (type_name == "int64") return *reinterpret_cast<const int64_t*>(src_ptr);
    else if (type_name == "uint64") return *reinterpret_cast<const uint64_t*>(src_ptr);
    else if (type_name == "float") return *reinterpret_cast<const float*>(src_ptr);
    else if (type_name == "double") return *reinterpret_cast<const double*>(src_ptr);
    else if (type_name == "string") return std::string(*reinterpret_cast<char* const*>(src_ptr));
    else if (type_name == "pointer") return reinterpret_cast<uintptr_t>(*reinterpret_cast<void* const*>(src_ptr));
    else if (struct_manager_.is_struct(type_name)) {
        const StructLayout* layout = struct_manager_.get_layout(type_name);
        if (!layout) {
            throw std::runtime_error("Struct layout not found for type: " + type_name);
...
void* FfiDispatcher::allocate_and_populate_arg(const json& arg_json, FfiArgs& arg_storage) {
    ...
    if (struct_manager_.is_struct(type_str)) {
        ...
        if (arg_json.contains("target_type")) {
            ...
            } else if (target_type_name.back() == ']') { // Check for array of structs, e.g., "Point[]"
        ...
        else if (type_str == "uint8") return arg_storage.allocate(arg_json.at("value").get<uint8_t>());
        else if (type_str == "int16") return arg_storage.allocate(arg_json.at("value").get<int16_t>());
        else if (type_str == "uint16") return arg_storage.allocate(arg_json.at("value").get<uint16_t>());
        else if (type_str == "int32") return arg_storage.allocate(arg_json.at("value").get<int32_t>());
        else if (type_str == "uint32") return arg_storage.allocate(arg_json.at("value").get<uint32_t>());
        else if (type_str == "int64") return arg_storage.allocate(arg_json.at("value").get<int64_t>());
        else if (type_str == "uint64") return arg_storage.allocate(arg_json.at("value").get<uint64_t>());
        else if (type_str == "float") return arg_storage.allocate(arg_json.at("value").get<float>());
        else if (type_str == "double") return arg_storage.allocate(arg_json.at("value").get<double>());
