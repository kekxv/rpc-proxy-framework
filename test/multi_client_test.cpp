...
protected:
    ...
    void SetUp() override {
        ...
        executor_thread_ = std::thread([this]() {
            {
                std::lock_guard<std::mutex> lock(g_test_log_mutex);
                std::cout << "[Test Main] Executor thread " << std::this_thread::get_id() << " started. Calling run()." << std::endl;
            }
...
bool run_client_session(int client_id, const std::string& lib_path) {
    {
        std::lock_guard<std::mutex> lock(g_test_log_mutex);
        std::cout << "[Client " << client_id << "][TID:" << std::this_thread::get_id() << "] Starting session." << std::endl;
    }
    ...
    if (call_resp["status"] != "success" || call_resp["data"]["return"]["value"] != (a + b)) {
        std::lock_guard<std::mutex> lock(g_test_log_mutex);
        std::cerr << "[Client " << client_id << "] Function call failed or returned wrong value: " << call_resp.dump(2) << std::endl;
        return false;
    }
